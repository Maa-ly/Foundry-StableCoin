...
contract Handler is Test {
    ...
    uint256 MAX_DEPOSIT_SIZE = type(uint96).max; // max number
    ...
    function redeemCollateral(
        ...
        ERC20Mock collateral = _getCollateralFromSeed(collateralSeed);
        uint256 maxCollateralToRedem = dsce.getCollateralBalanceOfUser(
            address(collateral),
            msg.sender
        );

        amountCollateral = bound(amountCollateral, 1, maxCollateralToRedem);
    ...
    ///function updateCollateralPrice(uint96) public {
